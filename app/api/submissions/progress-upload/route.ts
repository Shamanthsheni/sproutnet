...
export async function POST(req: Request) {
  ...
  if (!(file instanceof File)) {
    return NextResponse.json({ error: 'Missing progress file.' }, { status: 400 })
  }
  ...
  if (!enrollment || enrollment.length === 0) {
    return NextResponse.json({ error: 'You must enroll before uploading progress.' }, { status: 403 })
  }
  ...
  const filePath = `${user.id}/${problemId}/${Date.now()}-${crypto.randomUUID()}-${sanitizeProblemProgressFileName(file.name)}`
